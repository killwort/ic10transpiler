...
internal class LoadBatch : BuiltinFunction
{
    ...
    public override string Name => "getBatch";
    ...
    public override IEnumerable<Error> ValidateCall(IResolutionScope scope, FunctionCall call)
    {
        ...
        if (!IsConstOrDefine(scope.Root, call.Arguments[2])) yield return new Error(call.Line, call.Column, "Mode argument should be constant", ErrorType.Semantic);
    }
}
