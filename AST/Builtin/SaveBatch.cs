...
internal class SaveBatch : BuiltinFunction
{
    ...
    public override string Name => "setBatch";
    ...
    public override IEnumerable<Op> EmitCall(IResolutionScope scope, FunctionCall call)
    {
        ...
        if (IsConstOrDefine(scope.Root, call.Arguments[2]))
            ...
            foreach (var l in call.Arguments[2].Emit(scope))
                yield return l;
        yield return new Instruction(call, "sb", ToOperand(call.Arguments[0], scope), ToOperand(call.Arguments[1], scope), valueOperand);
    }
