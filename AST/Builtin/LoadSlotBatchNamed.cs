...
internal class LoadSlotBatchNamed : BuiltinFunction
{
    ...
    public override IEnumerable<Op> EmitCall(IResolutionScope scope, FunctionCall call)
    {
        ...
        if (IsConstOrDefine(scope.Root, call.Arguments[2]))
            slotIdOperand = ToOperand(call.Arguments[2]);
        else
            foreach (var l in call.Arguments[2].Emit(scope))
                yield return l;

        yield return new Instruction(call, "lbns", Acc0Operand.Instance, ToOperand(call.Arguments[0]), ToOperand(call.Arguments[1]), slotIdOperand, ToOperand(call.Arguments[3]), ToOperand(call.Arguments[4]));
    }
